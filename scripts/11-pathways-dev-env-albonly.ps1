...
aws cloudformation delete-stack --region $REGION --profile $PROFILE --stack-name pathways-mm-app-dev-env-1
Write-Output 'Waiting for the stack to be deleted, this may take a few minutes...'
aws cloudformation wait stack-delete-complete --region $REGION --profile $PROFILE --stack-name pathways-mm-app-dev-env-1
Write-Output 'Done'
...
aws cloudformation create-stack --region $REGION --profile $PROFILE --stack-name pathways-mm-app-dev-env-1 --template-url https://$S3_BUCKET.s3.$REGION.amazonaws.com/$CFN_TEMPLATE_ENV --parameters ParameterKey=DBName,ParameterValue=$DBName ParameterKey=ClusterName,ParameterValue=pathways-mm-app-dev-env-1 ParameterKey=LbName,ParameterValue=pathways-mm-app-dev-env-1 ParameterKey=TemplateBucket,ParameterValue=$S3_BUCKET ParameterKey=EnvironmentInstance,ParameterValue=$AppEnv --capabilities CAPABILITY_NAMED_IAM, CAPABILITY_AUTO_EXPAND
